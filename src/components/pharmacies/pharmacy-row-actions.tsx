...
import { deletePharmacy } from "@/actions/pharmacies";
import { Pencil, Trash2, MoreHorizontal, Loader2, X, Eye } from "lucide-react";
...
export function PharmacyRowActions({ pharmacyId, pharmacyName }: Props) {
  ...
  return (
    <>
      <button
        ...
        title="الإجراءات"
        id={`pharmacy-actions-${pharmacyId}`}
      >
      ...
      {showMenu && (
        <>
          ...
          <div
            className="fixed z-50 w-44 bg-card border border-border rounded-xl shadow-xl overflow-hidden py-1"
          ...
          >
            ...
            <Link
              href={`/dashboard/pharmacies/${pharmacyId}/edit`}
              onClick={() => setShowMenu(false)}
              className="flex items-center gap-2.5 px-3 py-2 text-sm text-foreground hover:bg-muted transition-colors w-full"
            >
        ...
        >
          <div className="bg-card border border-border rounded-2xl shadow-xl w-full max-w-sm p-6" dir="rtl">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              هل أنت متأكد من حذف الصيدلية <span className="font-semibold text-foreground">«{pharmacyName}»</span>؟
            </p>
